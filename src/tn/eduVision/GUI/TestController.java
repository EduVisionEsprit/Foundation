...
import tn.eduVision.services.QuestionService;
...
public class TestController implements Initializable {
    ...
    private Tab stat;
    @FXML
    private TableView<?> tvNote;
    @FXML
    private TableColumn<?, String> colSujetNote;
    @FXML
    private TableColumn<?, String> colEtudStat;
    @FXML
    private TableColumn<?, Integer> colNoteStat;
    @FXML
    private TableColumn<?, String> colFormation;
    @FXML
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        comboMinUpdate.setVisible(false);//To change body of generated methods, choose Tools | Templates.
    }
    ...
    private void addQuestion(ActionEvent event) {
        ...
        if (comboType.getValue() == null) {
            ...
            if (comboType.getValue().toLowerCase().equals("test")) {
                if (comboHeure.getValue() == null || comboMinutes.getValue() == null) {
    ...
    private void addTest(ActionEvent event) {
        if (q.getQuestions().isEmpty() || q.getQuestions().size() < 3) {
            ...
            if (comboType.getValue().toLowerCase().equals("quiz")) {
                ...
                qdao.insertQuiz(q, 1);         //currentUser.getId()
                showAlertMessageInfo("Ajouter Quiz", "Quiz/Test ajouter avec succès");
            }

            if (comboType.getValue().toLowerCase().equals("test")) {
                TestService tdao = new TestService();
                tdao.insertTest((Test) q, 1);       //currentUser.getId()  
    ...
    public void showTest() {
        QuizService qdao = new QuizService();
        ObservableList<Quiz> allQuiz = qdao.displayQuizList(1); //currentUser.getId()

        TestService tdao = new TestService();
        ObservableList<Test> listTest = tdao.displayTestList(1); //currentUser.getId()
        ...
        if (option == null) {
            ...
            if (option.toLowerCase().equals("question")) {
                ...
                if (!qPosee.isEmpty() && !rCorrecte.isEmpty() && !mRep1.isEmpty() && !mRep2.isEmpty() && !mRep3.isEmpty()
                    ...
                    QuestionService qqdao = new QuestionService();
            ...
            if ((option.toLowerCase().equals("quiz")) || (option.toLowerCase().equals("test"))) {
                if (tfSujetShow.getText().isEmpty()) {
                    ...
                    if (showSelectedQuiz(e).getClass().getSimpleName().toLowerCase().equals("quiz")) {
                        ...
                        QuizService qdao = new QuizService();
                        if (qdao.updateQuiz(quiz, 1)) {   //currentUser.getId()
                        ...
                        test.setDuree(duree);
                        TestService tdao = new TestService();
                        if (tdao.updateTest(test, 1)) { //currentUser.getId()
    ...
    private void setDataToLineChart(ObservableList<?> list) {
